...
// Usa ApiClient (que maneja tokens Bearer y refresco de sesión automáticamente).
...
async function loadAll(): Promise<Publication[]> {
    const response = await ApiClient.get<ApiResponse<Publication[]>>('/api/publications');
...
async function create(
    ...
    const response = await ApiClient.post<ApiResponse<Publication>>('/api/publications', formData);
...
async function remove(id: string): Promise<void> {
    await ApiClient.delete<ApiResponse<string>>(`/api/publications/${id}`);
}
